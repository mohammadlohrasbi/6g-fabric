...
// GeoSignalStrength defines the Smart Contract structure
type GeoSignalStrength struct {
	contractapi.Contract
}

// Antenna represents an antenna with coordinates and signal strength
type Antenna struct {
	ID            string  `json:"id"`
	X             float64 `json:"x"`
	Y             float64 `json:"y"`
...
// RegisterAntenna registers an antenna with its coordinates and signal strength
func (s *GeoSignalStrength) RegisterAntenna(ctx contractapi.TransactionContextInterface, antennaID string, x, y, signalStrength float64) error {
	antenna := Antenna{
		ID:            antennaID,
		X:             x,
		Y:             y,
...
func (s *GeoSignalStrength) RegisterUser(ctx contractapi.TransactionContextInterface, userID string, x, y float64) error {
	user := User{
		ID:        userID,
		X:         x,
		Y:         y,
...
// AssignBySignalStrength assigns a user to the antenna with the best signal strength
func (s *GeoSignalStrength) AssignBySignalStrength(ctx contractapi.TransactionContextInterface, userID string) error {
	...
	for iterator.HasNext() {
		...
		score := antenna.SignalStrength / distance
		if score > maxScore {
			maxScore = score
...
func (s *GeoSignalStrength) QueryAssignment(ctx contractapi.TransactionContextInterface, userID string) (string, error) {
	userBytes, err := ctx.GetStub().GetState("USER_" + userID)
	if err != nil {
		return "", fmt.Errorf("failed to read user %s: %v", userID, err)
	}
...
func main() {
	chaincode, err := contractapi.NewChaincode(&GeoSignalStrength{})
	if err != nil {
		fmt.Printf("Error creating GeoSignalStrength chaincode: %v", err)
		return
	}
	if err := chaincode.Start(); err != nil {
		fmt.Printf("Error starting GeoSignalStrength chaincode: %v", err)
	}
}
