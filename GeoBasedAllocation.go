...
// GeoBasedAllocation defines the Smart Contract structure
type GeoBasedAllocation struct {
...
func (s *GeoBasedAllocation) RegisterAntenna(ctx contractapi.TransactionContextInterface, antennaID string, x, y float64) error {
	antenna := Antenna{
		ID: antennaID,
		X:  x,
		Y:  y,
...
func (s *GeoBasedAllocation) RegisterUser(ctx contractapi.TransactionContextInterface, userID string, x, y float64) error {
	user := User{
		ID:        userID,
		X:         x,
		Y:         y,
...
// AssignToNearestAntenna assigns a user to the nearest antenna based on Euclidean distance
func (s *GeoBasedAllocation) AssignToNearestAntenna(ctx contractapi.TransactionContextInterface, userID string) error {
...
func (s *GeoBasedAllocation) QueryAssignment(ctx contractapi.TransactionContextInterface, userID string) (string, error) {
	userBytes, err := ctx.GetStub().GetState("USER_" + userID)
	if err != nil {
		return "", fmt.Errorf("failed to read user %s: %v", userID, err)
	}
...
func main() {
	chaincode, err := contractapi.NewChaincode(&GeoBasedAllocation{})
	if err != nil {
		fmt.Printf("Error creating GeoBasedAllocation chaincode: %v", err)
		return
	}
	if err := chaincode.Start(); err != nil {
		fmt.Printf("Error starting GeoBasedAllocation chaincode: %v", err)
	}
}
